...
import { directors, type Director } from '../data/mockDirectors';
import { formatCount } from '../lib/format';
import { rankDirectors } from '../lib/sort';

const topPerformers = rankDirectors(directors, 'performance').slice(0, 6);
const mostWatched = rankDirectors(directors, 'popularity').slice(0, 6);
const featured = ['priya-nadari', 'elena-cho', 'theo-marchetti']
  .map((id) => directors.find((director) => director.id === id))
  .filter((director): director is Director => Boolean(director));
const sectors = [...new Set(directors.map((director) => director.sectorGroup))];
  ...
  return (
    <div className="relative mx-auto h-[28rem] w-full max-w-[31rem] sm:h-[31rem]">
      {featured.map((director, index) => (
        ...
        >
          <Avatar name={director.name} seed={director.id} size="sm" company={director.company} ticker={director.ticker} />
          ...
          <ReturnValue value={director.absoluteReturnPct} className="text-[0.92rem]" />
        </Link>
      ))}
      <div className="absolute bottom-0 right-0 rounded-2xl bg-btn px-5 py-4 text-btn-fg shadow-card">
        ...
        <p className="mt-1 text-[0.95rem] font-semibold">15 fictional profiles</p>
      </div>
    </div>
  );
}
  ...
  return (
    <Link to={`/director/${director.id}`} className="group flex min-h-[16rem] flex-col justify-between rounded-2xl bg-surface-2 p-5 transition-colors hover:bg-[#ececf0] md:p-6">
      <div className="flex items-start justify-between gap-4">
        <Avatar name={director.name} seed={director.id} size="lg" company={director.company} ticker={director.ticker} />
        <ReturnValue value={director.absoluteReturnPct} className="text-[1.25rem]" />
      </div>
      <div className="mt-10">
...
function RankedList({ people, metric }: { people: Director[]; metric: 'performance' | 'watching' }) {
  return (
    <div className="divide-y divide-line border-y border-line">
      {people.map((director, index) => (
        <Link key={director.id} to={`/director/${director.id}`} className="grid grid-cols-[2rem_auto_minmax(0,1fr)_auto] items-center gap-3 py-3.5 transition-colors hover:bg-surface-2">
          ...
          {metric === 'performance' ? <ReturnValue value={director.absoluteReturnPct} className="text-[0.94rem]" /> : <span className="tnum text-[0.87rem] font-semibold text-ink">{formatCount(director.watcherCount)} <span className="hidden text-[0.72rem] font-medium text-ink-soft sm:inline">watching</span></span>}
        </Link>
      ))}
    </div>
  );
      ...
      <main className="mx-auto w-full max-w-[1280px] flex-1 px-5 pb-16 md:px-8 lg:px-10">
        ...
        <section className="grid items-center gap-12 pb-20 pt-16 md:pt-24 lg:min-h-[calc(100vh-4.5rem)] lg:grid-cols-[1fr_0.9fr] lg:gap-20 lg:py-16">
          <div className="max-w-[42rem]">
            ...
            <p className="mt-7 max-w-[34rem] text-[1rem] leading-relaxed text-ink-soft md:text-[1.13rem]">Explore fictional profiles of directors, CEOs, and CFOs through the trades they disclose and the timing behind them.</p>
            ...
            <p className="mt-5 text-[0.71rem] leading-relaxed text-ink-faint">All profiles, trades, companies, and performance figures are fictional and for demonstration only.</p>
          </div>
          <StackedProfiles />
        </section>
        ...
        <section className="grid gap-16 border-t border-line py-16 md:py-24 lg:grid-cols-2 lg:gap-20">
          <div><h2 className="text-[clamp(2rem,3.4vw,3.15rem)] font-bold tracking-[-0.035em] text-ink">Top performers</h2><p className="mt-2 max-w-[27rem] text-[0.94rem] text-ink-soft">Ranked by fictional performance since each profile’s first tracked trade.</p><div className="mt-7"><RankedList people={topPerformers} metric="performance" /></div><Link to="/discover" className="mt-5 inline-flex items-center gap-2 text-[0.84rem] font-semibold text-ink underline decoration-line-strong underline-offset-4">See full leaderboard <Arrow /></Link></div>
          <div><h2 className="text-[clamp(2rem,3.4vw,3.15rem)] font-bold tracking-[-0.035em] text-ink">Most watched</h2><p className="mt-2 max-w-[27rem] text-[0.94rem] text-ink-soft">The profiles the fictional Insider Index community keeps close.</p><div className="mt-7"><RankedList people={mostWatched} metric="watching" /></div><Link to="/discover" className="mt-5 inline-flex items-center gap-2 text-[0.84rem] font-semibold text-ink underline decoration-line-strong underline-offset-4">Explore the index <Arrow /></Link></div>
        </section>

        <section className="border-t border-line py-16 md:py-24"><div className="grid gap-10 lg:grid-cols-[0.7fr_1.3fr] lg:gap-20"><div><h2 className="text-[clamp(2rem,3.4vw,3.15rem)] font-bold leading-[0.98] tracking-[-0.035em] text-ink">Explore the signal from every angle.</h2></div><div><p className="max-w-[40rem] text-[1.05rem] leading-relaxed text-ink-soft">Every Insider Index profile comes with a company context, trade history, current holdings, and a comparison of performance against the market and their own stock.</p><div className="mt-8 flex flex-wrap gap-2">{sectors.map((sector) => <Link key={sector} to="/discover" className="rounded-full border border-line-strong px-4 py-2 text-[0.8rem] font-semibold text-ink transition-colors hover:bg-surface-2">{sector}</Link>)}</div></div></div></section>

        <section id="how-it-works" className="border-t border-line py-16 md:py-24"><h2 className="text-[clamp(2rem,3.4vw,3.15rem)] font-bold tracking-[-0.035em] text-ink">How it works</h2><div className="mt-9 grid gap-10 md:grid-cols-3 md:gap-8"><div><p className="text-[1.15rem] font-bold text-ink">Find a profile</p><p className="mt-3 text-[0.92rem] leading-relaxed text-ink-soft">Search the index by person, company, ticker, or sector.</p></div><div><p className="text-[1.15rem] font-bold text-ink">Read the context</p><p className="mt-3 text-[0.92rem] leading-relaxed text-ink-soft">Look beyond the headline return with holdings, company prices, and every plotted trade.</p></div><div><p className="text-[1.15rem] font-bold text-ink">Keep a watchlist</p><p className="mt-3 text-[0.92rem] leading-relaxed text-ink-soft">Save the fictional profiles you want to revisit. Everything stays local to this demo.</p></div></div></section>

        <footer className="border-t border-line pt-10 text-[0.74rem] leading-relaxed text-ink-faint md:flex md:items-end md:justify-between"><div><div className="flex items-center gap-2 text-ink"><Mark /><span className="flex items-center gap-1 font-bold">Insider Index <BlueCheck className="h-4 w-4" /></span></div><p className="mt-4 max-w-[35rem]">A fictional, illustrative prototype for exploring the structure of corporate insider activity. It is not an investment product and does not place trades.</p></div><p className="mt-5 md:mt-0">© 2026 Insider Index</p></footer>
      </main>
      <BottomNav />
    </>
  );
